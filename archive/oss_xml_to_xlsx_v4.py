#!/usr/bin/env python3
"""
OSS XML to XLSX Converter  — Version 4  (Blazing Fast + Overlap Parse/UI)
Converts Nokia OSS RAML XML dump files to Excel format.

Key improvements over V3:
  - Parsing starts immediately after class selection, in a background thread
  - "Save as" dialog runs concurrently with parsing — zero idle time
  - By the time the user picks an output path, parsing is often already done

Key improvements over V2 (carried from V3):
...
Usage (interactive GUI):   python oss_xml_to_xlsx_v4.py
Usage (command-line):      python oss_xml_to_xlsx_v4.py file1.xml.gz file2.gz -o out.xlsx
                           python oss_xml_to_xlsx_v4.py file1.zip --classes BTS,BCF -o out.xlsx
"""
...
def parse_input_file(filepath, filter_classes=None, n_workers=1):
    ...
    for xml_bytes, display_name in iter_xml_streams(filepath):
        results.append(parse_xml_bytes_v3(xml_bytes, display_name,
                                          filter_classes, n_workers))
    return results
...
def generate_info_sheet_xml(files_info, class_names):
    ...
    # Row 2: title
    parts.append(f'<row r="{row}">')
    parts.append(_cell_xml(1, row, 'Created with OSS XML Converter  --  V4', style=1))
    parts.append('</row>')
    row += 1
...
def convert_to_xlsb(xlsx_path, xlsb_path):
    ...
    excel.Visible       = False   # keep window hidden
    excel.DisplayAlerts = False   # suppress all pop-ups / save confirmations
    excel.ScreenUpdating = False  # prevent any screen redraws flashing through
    try:
...
def _write_phase(merged, ordered_fi, output_path, t_parse, progress=None):
    ...
    if want_xlsb:
        tmp_fd, xlsx_path = tempfile.mkstemp(prefix='oss_v4_', suffix='.xlsx')
        os.close(tmp_fd)
    else:
    ...
    if want_xlsb:
        ...
        try:
            ...
            # Always remove the temp xlsx — even if Excel crashes mid-conversion
            try:
                os.remove(xlsx_path)
            except OSError:
                pass
    ...
    print(line)
    return output_path, t_write, t_asm, t_xlsb
...
def _ask_summary_dialog(root, has_4g, has_2g):
    ...
    if has_4g:
        tk.Checkbutton(dlg,
                       text='  4G Summary   (LNCEL data found)',
                       variable=var_4g,
                       font=('Calibri', 10)).pack(padx=28, anchor='w')
    if has_2g:
        tk.Checkbutton(dlg,
                       text='  2G Summary   (BTS data found)',
                       variable=var_2g,
                       font=('Calibri', 10)).pack(padx=28, anchor='w')
...
def _post_process_summaries(output_file, class_names, root):
    ...
    has_4g = bool(class_set & {'LNCEL', 'LNCEL_FDD', 'LNCEL_TDD'})
    has_2g = 'BTS' in class_set
...
def run_interactive():
    ...
    # The "Save as" dialog below can take 10–30 s; we parse during that time.
    ...
    def _bg_parse():
        try:
            # No progress passed here — save dialog blocks tkinter event loop
            # so any updates would queue up invisibly anyway.
            merged, ordered_fi, t_parse = _parse_phase(input_paths, filter_classes)
    ...
    # ── Step 5: Ask for output path (runs while parsing is happening) ─────────
    # NOTE: native Windows file dialogs block the tkinter event loop entirely,
    # so we intentionally do NOT show the progress window until after this closes.
    ...
    # ── Step 8: Write + assemble ──────────────────────────────────────────────
    t_wall = datetime.now()
    output_path, t_write, t_asm, t_xlsb = _write_phase(
        _parse_result['merged'], _parse_result['fi'],
        output_path, _parse_result['t_parse'], progress=progress)
    t_wall = (datetime.now() - t_wall).total_seconds()
    print(f'         Wall clock total (from parse start): {fmt_elapsed(t_wall)}')
    ...
    if progress:
        ...
        t_total = t_parse + t_write + t_asm + t_xlsb
        size_mb = os.path.getsize(output_path) / 1024 / 1024
        status = (f'{os.path.basename(output_path)}  ({size_mb:.1f} MB)\n'
                  f'Parse {fmt_elapsed(t_parse)}  |  '
                  f'Write {fmt_elapsed(t_write)}  |  '
                  f'Assemble {fmt_elapsed(t_asm)}')
        if t_xlsb:
            status += f'  |  XLSB {fmt_elapsed(t_xlsb)}'
        status += f'  |  Total {fmt_elapsed(t_total)}'
        progress.done(phase='Done!', status=status)
        progress.wait()

    # ── Step 10: Optional 2G / 4G summary generation ─────────────────────────
    t_4g, t_2g = _post_process_summaries(output_path, class_names, root)

    # ── Grand total line ──────────────────────────────────────────────────────
    t_parse = _parse_result['t_parse']
    t_grand = t_parse + t_write + t_asm + t_xlsb + t_4g + t_2g
    grand_line = (f'\n[{ts()}] GRAND TOTAL: {fmt_elapsed(t_grand)}'
                  f'  (Parse: {fmt_elapsed(t_parse)}'
                  f'  |  Write: {fmt_elapsed(t_write)}'
                  f'  |  Assemble: {fmt_elapsed(t_asm)}')
    if t_xlsb:
...
def main():
    os.system("title Ankit's XML Parser  [V4 - Blazing Fast + Overlap Parse/UI]")
    ...
    parser = argparse.ArgumentParser(description='OSS XML -> Excel Converter V4 (Blazing Fast + Overlap Parse/UI)')
