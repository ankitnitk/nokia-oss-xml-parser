#!/usr/bin/env python3
"""
OSS XML to XLSX Converter  — Version 6.0  (SST + Column-Order Cache)
Converts Nokia OSS RAML XML dump files to Excel format.

Key improvements over V5.1:
  - Shared String Table (SST): repeated string values (enums, versions, filenames)
    are stored once in xl/sharedStrings.xml; each cell reference shrinks from
    ~60 chars (inlineStr) to ~25 chars (t="s").  For Nokia dumps with heavy
    enum/boolean repetition this cuts worksheet XML size 30–60 % and compresses
    better, giving faster write + smaller output files.
  - Column-order cache: build_column_order() (which scans every record) is called
    once during the SST pre-scan rather than a second time inside the write loop.
    For wide classes (100+ columns, millions of rows) this saves one full pass.
  - SST is built with a count-based filter: only strings that appear MORE THAN ONCE
    across all records are stored in the SST. Unique identifiers like Dist_Name
    that appear exactly once per record are left as inlineStr — this keeps the
    pickled sst_dict small (typically < 10 000 entries) so IPC to worker processes
    stays fast even on very large dumps.
...
Usage (interactive GUI):   python oss_xml_to_xlsx_v6.0.py
Usage (command-line):      python oss_xml_to_xlsx_v6.0.py file1.xml.gz file2.gz -o out.xlsx
                           python oss_xml_to_xlsx_v6.0.py file1.zip --classes BTS,BCF -o out.xlsx
"""
...
def generate_info_sheet_xml(files_info, class_names):
    ...
    # Row 2: title
    parts.append(f'<row r="{row}">')
    parts.append(_cell_xml(1, row, 'Created with OSS XML Converter  --  V6.0', style=1))
    parts.append('</row>')
    row += 1
...
def write_sheet_worker(args):
    ...
    V6.0: args tuple has 6 elements — includes sst_dict for SST cell generation.
    """
...
# ---------------------------------------------------------------------------
# Phase 2: Write + Assemble  (V6.0 — SST pre-scan + col-order cache)
# ---------------------------------------------------------------------------
...
def _write_phase(merged, ordered_fi, output_path, t_parse, progress=None,
    ...
    V6.0 key change: a single SST pre-scan loop iterates all records ONCE
    to (a) build column order for every class and (b) count string occurrences.
    Only strings appearing more than once are added to the SST and written to
    xl/sharedStrings.xml — keeping sst_dict small so worker-process IPC is fast.
    Column orders are cached; the write loop reuses them without re-scanning.
...
class ProgressWindow:
    ...
    def __init__(self, parent):
        ...
        self.win.title("OSS XML Parser  V6.0")
...
def main():
    os.system("title Ankit's XML Parser  [V6.0 - SST + Streaming]")
    ...
    parser = argparse.ArgumentParser(description='OSS XML -> Excel Converter V6.0 (SST + Streaming)')
