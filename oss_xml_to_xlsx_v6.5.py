#!/usr/bin/env python3
"""
OSS XML to XLSX Converter  — Version 6.5  (Streaming Parse)
Converts Nokia OSS RAML XML dump files to Excel format.

Key fix over V6.4:
  - Crash fix: try_numeric() / parse_dist_name() called int(float(s)) on every
    numeric-looking parameter value. Some values are alphanumeric text that
    still happens to be valid float *syntax* (e.g. a hardware serialNumber
...
Key improvement over V6.3:
  - Streaming parse: the gzip stream is decompressed and scanned in 64 MB
    chunks; MO blocks are carved out of each chunk (as bytes) and dispatched
    to worker processes WHILE decompression continues.  The full multi-GB
    document is never decoded nor held in memory — peak RAM is O(chunk +
    in-flight batches) instead of O(document), and gunzip/scan time overlaps
    worker parse time instead of preceding it.

Key improvement over V6.2:
  - Single-pass MO block slicing: the parse phase previously made three full
    passes over the decompressed XML — a regex .sub() to normalise self-closing
    <managedObject/> tags (rebuilding the entire multi-GB string), then
    _MO_SPLIT.split(), then a class filter loop.  V6.3 replaces all three with
    one _MO_FIND_RE.finditer() scan that slices each MO as text[open:next_open]
    and filters by class inline, so only the kept blocks are ever materialised.
    ~2.2× faster block extraction (e.g. 22 s → 10 s per file on a 3.4 GB dump),
    byte-identical output.  Also bundles the 4G InterFreq HO Check (4g_tool).

Key improvements over V6.0:
  - Sparse flatten: flatten_records_sparse() iterates the actual keys present
    in each (hier, rec) pair instead of probing every column position via
    dict.get().  Nokia MO records are typically sparse — a class with 200
    columns may have only 25–40 filled params per record.  This cuts the
    per-record work from O(n_cols) to O(filled_keys), which is 4–8× fewer
    dict lookups for wide sparse classes.
  - dict instead of OrderedDict: parse_dist_name and parse_mo_block now use
    plain dicts.  Python 3.7+ guarantees insertion-order preservation so the
    output is identical, but plain dicts have lower allocation and pickle
    overhead than the OrderedDict subclass.

Key improvements over V5.1 (carried from V6.0):
  - Shared String Table (SST): repeated string values stored once in
    xl/sharedStrings.xml; cell XML shrinks from ~60 chars (inlineStr) to
    ~25 chars (t="s"), cutting worksheet XML size 30–60 %.
  - Column-order cache: records scanned once (SST pre-scan) for both column
    schema and string counts — write loop reuses the cache.

Key improvements over V5.0 (carried from V5.1):
  - Streaming worksheet XML: rows written directly to disk in batches — peak RAM
    stays constant regardless of sheet size.
  - Streaming ZIP assembly: sheets copied into the xlsx zip in chunks.

Key improvements over V4.2 (carried from V5):
  - Pre-read snapshot: summary tools read from memory, never re-read the xlsx.
  - Parallel XML files in ZIP: multiple XMLs inside one archive parsed simultaneously.
  - Both dialogs (summary + save-as) shown during parsing.
  - XLSB pre-warm: Excel.Application launched before write.
  - XLSB + summaries run in parallel; 4G and 2G summaries also run in parallel.
  - Grand Total shows true wall-clock time.

Usage (interactive GUI):   python oss_xml_to_xlsx_v6.1.py
Usage (command-line):      python oss_xml_to_xlsx_v6.1.py file1.xml.gz file2.gz -o out.xlsx
                           python oss_xml_to_xlsx_v6.1.py file1.zip --classes BTS,BCF -o out.xlsx
"""
...
# (V6.2 normalised these to paired open+close before splitting; V6.3 no longer
# needs that rewrite — see _MO_FIND_RE below.  Kept for reference.)
_MO_SELF_CLOSE_RE = re.compile(r'(<(?:\w+:)?managedObject\s[^>]*)/>')

# V6.3: single-pass MO block slicing (replaces self-close-sub + split + filter).
# _MO_FIND_RE matches every managedObject OPEN tag — both paired
# <managedObject ...> and self-closing <managedObject .../> (the '[^>]*>' run
# swallows the trailing '/' of a self-closing tag).  Each MO block is then
# text[this_open : next_open], so a self-closing empty MO becomes its own
# complete block with no string rewrite.  _CLASS_IN_ATTRS pulls the class out
# of a matched open tag for in-scan filtering.
_MO_FIND_RE     = re.compile(r'<(?:\w+:)?managedObject\s[^>]*>')
_CLASS_IN_ATTRS = re.compile(r'class="([^"]+)"')

# V6.4: bytes-level equivalents for the streaming parser.  The gzip stream is
# scanned as raw bytes chunk-by-chunk — the full document is never decoded (or
# even fully held) in memory; each kept block is decoded individually in the
# worker process.
_MO_FIND_B  = re.compile(rb'<(?:\w+:)?managedObject\s[^>]*>')
_CLASS_B    = re.compile(rb'class="([^"]+)"')
_LOG_DT_B   = re.compile(rb'<(?:\w+:)?log\s[^>]*dateTime="([^"]*)"')

_STREAM_CHUNK  = 64 << 20   # 64 MB decompressed per read
_BATCH_BLOCKS  = 6000       # kept blocks per worker submission
...
def parse_xml_bytes_v3(data, display_name, filter_classes=None, n_workers=1):
    ...
    # ── Slice MO blocks in a single finditer pass (V6.3) ─────────────────────
    # Replaces V6.2's self-close-sub + _MO_SPLIT.split + filter loop (three
    # full-text passes — ~23 s/file on a 3.4 GB decompressed string).  Here one
    # scan over managedObject open-tags yields each block as
    # text[this_open : next_open]; a self-closing <managedObject .../> becomes
    # its own complete block (no rewrite, so the old MAL-10 / SMLC data-loss
    # bug stays fixed), and class filtering happens during the scan so only
    # kept blocks are ever sliced.  Verified byte-identical to V6.2 output.
    filter_set = filter_classes if filter_classes else None
    all_blocks = []
    pending = None   # (start_offset, keep?) of the previously matched open tag
    for m in _MO_FIND_RE.finditer(text):
        if pending is not None and pending[1]:
            all_blocks.append(text[pending[0]:m.start()])
        if filter_set:
            cm   = _CLASS_IN_ATTRS.search(m.group(0))
            keep = (cm is not None and cm.group(1) in filter_set)
        else:
            keep = True
        pending = (m.start(), keep)
    if pending is not None and pending[1]:
        all_blocks.append(text[pending[0]:])
...
def _parse_blocks_worker_bytes(args):
    """
    Runs in a worker process.  V6.4 variant: blocks arrive as BYTES, already
    class-filtered by the streaming scanner, so no re-filter here.  Each block
    is decoded individually — the full document is never decoded.
    args = (blocks: list[bytes], display_name: str)
    """
    blocks, display_name = args
    classes_data = {}
    for block in blocks:
        result = parse_mo_block(block.decode('utf-8', errors='replace'),
                                display_name)
        if result is None:
            continue
        mo_class, hierarchy, record = result
...
def parse_xml_stream(fobj, display_name, filter_classes=None, n_workers=1):
    """
    V6.4 streaming parser: reads the (decompressed) XML from *fobj* in 64 MB
    chunks and never materialises the whole document.

    Pipeline per chunk:
      carry + chunk → finditer managedObject open-tags → slice kept blocks
      (class filter inline, as bytes) → batch → submit to worker processes.

    Workers parse WHILE the main thread is still decompressing/scanning the
    next chunk, so gunzip+scan time overlaps parse time instead of preceding
    it.  Peak RAM is O(chunk + in-flight batches), not O(document).

    The executor is created lazily on the first full batch, so small files
    (< _BATCH_BLOCKS kept blocks) parse inline with zero process-spawn cost.
    """
    ...
    filter_bset = ({c.encode() for c in filter_classes}
                   if filter_classes else None)
    classes_data = defaultdict(list)
    ...
    def _flush(final=False):
        ...
        if not batch:
            return
        if ex is None and not final and n_workers > 1:
            ex = ProcessPoolExecutor(max_workers=n_workers)
        if ex is not None:
            while len(pending) >= max_inflight:
                _merge(pending.popleft().result())
            pending.append(ex.submit(_parse_blocks_worker_bytes,
                                     (batch, display_name)))
            batch = []
        else:
            _merge(_parse_blocks_worker_bytes((batch, display_name)))
            batch = []

    carry = b''
    first_chunk = True
    while True:
        chunk = fobj.read(_STREAM_CHUNK)
        if not chunk:
            break
        buf = carry + chunk
        if first_chunk:
            log_m = _LOG_DT_B.search(buf[:1 << 20])
            if log_m:
                file_info['dateTime'] = log_m.group(1).decode('utf-8', 'replace')
            first_chunk = False

        # Slice blocks between consecutive open-tags (same logic as V6.3, on
        # bytes).  The final open-tag onward is carried into the next chunk —
        # unless that MO is filtered out, in which case its bytes are dropped
        # immediately and scanning resumes at the next open tag.
        prev = None   # (start_offset, keep?)
        for m in _MO_FIND_B.finditer(buf):
            if prev is not None and prev[1]:
                batch.append(buf[prev[0]:m.start()])
                n_kept += 1
                if len(batch) >= _BATCH_BLOCKS:
                    _flush()
            if filter_bset is not None:
                cm   = _CLASS_B.search(m.group(0))
                keep = (cm is not None and cm.group(1) in filter_bset)
            else:
                keep = True
            prev = (m.start(), keep)

        if prev is None:
            carry = buf          # no open tag yet (file header) — keep waiting
        elif prev[1]:
            carry = buf[prev[0]:]   # last MO incomplete — rescan next round
        else:
            carry = b''             # last MO filtered out — drop its bytes

    if carry and _MO_FIND_B.search(carry):
        # Final MO block (runs to EOF, incl. any trailing </cmData> footer —
        # parse_mo_block ignores content after the block, same as V6.3).
        batch.append(carry)
        n_kept += 1
    _flush(final=(ex is None))
    while pending:
        _merge(pending.popleft().result())
    if ex is not None:
        ex.shutdown()
    ...
    count   = sum(len(v) for v in classes_data.values())
    elapsed = (datetime.now() - t0).total_seconds()
    mode    = (f'{n_workers}-core streamed' if ex is not None
               else '1-core streamed')
    tprint(f'  [{ts()}] Done    {display_name} -- {count:,} objects '
           f'in {fmt_elapsed(elapsed)}  [{mode}]')
    return classes_data, file_info


def _open_binary_streams(filepath):
    """
    Yield (fobj, display_name, closer) binary streams for every XML reachable
    from filepath.  .gz and .xml stream straight from disk (no full read);
    ZIP containers fall back to in-memory BytesIO (rare + typically smaller).
    """
    name = os.path.basename(filepath)
    low  = filepath.lower()
    if low.endswith('.zip'):
        ...
        if not found:
            raise ValueError(f'No XML files found inside {name}')
        for xml_bytes, dn in found:
            yield io.BytesIO(xml_bytes), dn, None
    elif low.endswith('.gz'):
...
def parse_input_file(filepath, filter_classes=None, n_workers=1):
    streams = list(_open_binary_streams(filepath))
    try:
        if len(streams) <= 1:
            # Single stream — give it the full core budget.
            return [parse_xml_stream(fobj, dn, filter_classes, n_workers)
                    for fobj, dn, _ in streams]

        # Multiple streams (e.g. a zip with 2+ XML files) — split the core
        # budget equally and parse all streams in parallel threads.
        workers_each = max(1, n_workers // len(streams))
        results = [None] * len(streams)

        def _stream_worker(idx, fobj, dn):
            results[idx] = parse_xml_stream(fobj, dn, filter_classes,
                                            workers_each)

        threads = [threading.Thread(target=_stream_worker,
                                    args=(i, fobj, dn), daemon=True)
                   for i, (fobj, dn, _) in enumerate(streams)]
        for t in threads:
            t.start()
        for t in threads:
            t.join()
...
def flatten_records_sparse(records, all_cols, hier_cols_set):
    """
    Convert (hier, rec) tuples to flat value lists for fast pickling to workers.

    V6.1 improvement over V6.0 flatten_records():
      The old version iterated every column position (dense): for each record it
      called dict.get() n_cols times regardless of how many params are actually
      present.  Nokia MO records are sparse — a class with 200 columns typically
      has only 25–40 filled params per record.

      This version iterates the actual keys in hier and rec (sparse).  For each
      key a single col_pos dict lookup maps it to its column index.  Only filled
      keys are visited, cutting per-record work from O(n_cols) to O(filled_keys).
      For wide sparse classes this is 4–8× fewer dict operations.
    """
    n_cols  = len(all_cols)
    col_pos = {col: i for i, col in enumerate(all_cols)}

    result = []
    for hier, rec in records:
        row = [None] * n_cols
...
def _cell_xml(col_idx, row_idx, value, style=0):
    """Generate XML for a single cell. row_idx is 1-based. Uses inlineStr (Info sheet)."""
...
def generate_worksheet_xml(cls_name, flat_records, all_cols, n_hier,
                            return_hyperlinks=True, sst_dict=None):
    """
    Generate raw worksheet XML for one sheet (in-memory version).
    Returns (sheet_xml_bytes, rels_xml_bytes_or_None).
    sst_dict: optional str→idx mapping for SST cells.
    """
    n_cols = len(all_cols)
    ...
    if sst_dict is not None:
        idx = sst_dict.get('Info', -1)
        if idx >= 0:
            parts.append(f'<c r="A1" s="2" t="s"><v>{idx}</v></c>')
        else:
    ...
    for ci, col_name in enumerate(all_cols):
        if sst_dict is not None:
            ...
            if idx >= 0:
                parts.append(f'<c r="{col_letters[ci]}2" s="3" t="s"><v>{idx}</v></c>')
                continue
        parts.append(_cell_xml(ci, 2, col_name, style=3))
    parts.append('</row>')
    ...
    for ri, row in enumerate(flat_records, start=3):
        ...
        for ci, val in enumerate(row):
            ...
            if isinstance(val, (int, float)):
                ...
                s = str(val)
                if sst_dict is not None:
                    idx = sst_dict.get(s, -1)
                    if idx >= 0:
                        row_parts.append(f'<c r="{ref}" t="s"><v>{idx}</v></c>')
                        continue
                if '&' in s or '<' in s or '>' in s or '"' in s:
                    s = (s.replace('&', '&amp;')
                          .replace('<', '&lt;')
    ...
    if return_hyperlinks:
        parts.append("<hyperlinks><hyperlink ref=\"A1\" location=\"'Info'!A1\"/></hyperlinks>")
...
def generate_info_sheet_xml(files_info, class_names):
    """Generate Info sheet XML directly (always uses inlineStr — Info sheet is tiny)."""
    parts = []
    parts.append('<?xml version="1.0" encoding="UTF-8" standalone="yes"?>')
    parts.append('<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"'
                 ' xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">')
    ...
    # Row 2: title
    parts.append(f'<row r="{row}">')
    parts.append(_cell_xml(1, row, 'Created with OSS XML Converter  --  V6.5', style=1))
    parts.append('</row>')
    row += 1
...
def _stream_worksheet_xml(xml_path, flat_records, all_cols, n_hier, sst_dict):
    """
    Write worksheet XML directly to xml_path, streaming rows in batches.

    V6.0: accepts sst_dict (str → global_idx).  String cells are written as
    t="s" SST references instead of verbose inlineStr elements.  Strings not
    found in sst_dict (unique identifiers like Dist_Name) fall back to inlineStr.
    ...
    with open(xml_path, 'w', encoding='utf-8', newline='', buffering=1 << 20) as f:
        ...
        # ── Sheet data ────────────────────────────────────────────────────────
        # Row 1: Info hyperlink — SST when available, inlineStr otherwise
        info_idx = sst_dict.get('Info', -1) if sst_dict else -1
        if info_idx >= 0:
            w(f'<sheetData><row r="1"><c r="A1" s="2" t="s"><v>{info_idx}</v></c></row>')
        else:
        ...
        for ci, col_name in enumerate(all_cols):
            if sst_dict is not None:
                idx = sst_dict.get(col_name, -1)
                if idx >= 0:
                    hdr.append(f'<c r="{col_letters[ci]}2" s="3" t="s"><v>{idx}</v></c>')
                    continue
            esc = _xml_escape(col_name)
            hdr.append(f'<c r="{col_letters[ci]}2" s="3" t="inlineStr"><is><t>{esc}</t></is></c>')
        hdr.append('</row>')
        w(''.join(hdr))

        # ── Data rows — batched ───────────────────────────────────────────────
        # Hot path notes (same as V5.1, extended for SST):
        # • col_letters: pre-computed once, avoids _col_letter() on every cell.
        # • ri_str: row index stringified once per row.
        # • SST lookup is a single dict.get() — O(1) per string cell.
        # • Strings absent from SST (unique identifiers) fall back to inlineStr.
        # • parts.extend per row — one C-level call.
        batch = []
        for ri, row in enumerate(flat_records, start=3):
            ri_str    = str(ri)
            row_parts = [f'<row r="{ri_str}">']
                ...
                if type(val) is int or type(val) is float:
                    ...
                    if sst_dict is not None:
                        ...
                        if idx >= 0:
                            row_parts.append(f'<c r="{ref}" t="s"><v>{idx}</v></c>')
                            continue
                    # Fallback: unique string not in SST → inlineStr
                    if '&' in s or '<' in s or '>' in s or '"' in s:
                        s = (s.replace('&', '&amp;')
                              .replace('<', '&lt;')
                              .replace('>', '&gt;')
...
def write_sheet_worker(args):
    ...
    V6.1: flat_records produced by flatten_records_sparse (sparse key iteration).
    """
    cls, flat_records, all_cols, n_hier, temp_dir, sst_dict = args
    t_start = datetime.now()

    safe_name = cls.replace('/', '_').replace('\\', '_')
    xml_path  = os.path.join(temp_dir, f'{safe_name}.xml')

    _stream_worksheet_xml(xml_path, flat_records, all_cols, n_hier, sst_dict)
...
def _shared_strings_xml(sst_list):
    """Build xl/sharedStrings.xml from the global string table."""
    n = len(sst_list)
    parts = [
    ...
    for s in sst_list:
        esc = _xml_escape(str(s))
        # xml:space="preserve" required if the string has leading/trailing whitespace
        if esc != esc.strip():
            parts.append(f'<si><t xml:space="preserve">{esc}</t></si>')
        else:
...
def _content_types_xml(n_sheets, has_sst=False):
    overrides = ''.join(
        f'<Override PartName="/xl/worksheets/sheet{i+1}.xml" ContentType="{WS_CT}"/>'
        for i in range(n_sheets)
    )
    sst_override = (f'<Override PartName="/xl/sharedStrings.xml" ContentType="{SST_CT}"/>'
                    if has_sst else '')
    return (
        '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
...
def _workbook_rels_xml(n_sheets, has_sst=False):
    ws_rels = ''.join(
        f'<Relationship Id="rId{i+1}" Type="{WS_REL}" Target="worksheets/sheet{i+1}.xml"/>'
        for i in range(n_sheets)
    )
    sty_id  = n_sheets + 1
    sty_rel = f'<Relationship Id="rId{sty_id}" Type="{STY_REL}" Target="styles.xml"/>'
    sst_rel = ''
    if has_sst:
        sst_id  = sty_id + 1
        sst_rel = f'<Relationship Id="rId{sst_id}" Type="{SST_REL}" Target="sharedStrings.xml"/>'
    return (
        '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
        '<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">'
        + ws_rels + sty_rel + sst_rel +
        '</Relationships>'
    ).encode()
...
def assemble_xlsx(sheet_order, sheet_paths, output_path, sst_list=None):
    """
    Build the final xlsx from worksheet XML files on disk.
    sheet_paths: {name: (xml_path_or_bytes, rels_path_or_None)}
    sst_list: global ordered list of unique strings (V6.0 SST).
...
# ---------------------------------------------------------------------------
# Phase 2: Write + Assemble  (V6.1 — Sparse flatten + dict parser + SST)
# ---------------------------------------------------------------------------
...
def _write_phase(merged, ordered_fi, output_path, t_parse, progress=None,
    ...
    V6.1 key changes:
      - flatten_records_sparse replaces flatten_records: iterates actual keys
        present in each (hier, rec) pair (O(filled_keys)) instead of probing
        every column via dict.get (O(n_cols)).  Wide sparse classes benefit most.
      - parse_mo_block and parse_dist_name use plain dicts instead of OrderedDict;
        Python 3.7+ preserves insertion order so output is identical, but plain
        dicts are cheaper to allocate and pickle.
      - SST pre-scan (from V6.0): single pass builds column orders + string counts.
    ...
    if want_xlsb:
        tmp_fd, xlsx_path = tempfile.mkstemp(prefix='oss_v6_', suffix='.xlsx')
        os.close(tmp_fd)
    else:
        xlsx_path = output_path

    class_names = sorted(merged.keys())
    temp_dir = tempfile.mkdtemp(prefix='oss_v6_')
    try:
        ...
        # ── Build sheet plan ─────────────────────────────────────────────────
        # Identify classes that need row-splitting BEFORE the SST scan so we
        # can print the correct sheet count in the progress message.
        sheet_plan = []
        for cls in class_names:
        ...
        # ── SST Pre-scan + Column-order cache ────────────────────────────────
        # Single pass over all records:
        #   • Inline build_column_order logic (avoids re-iterating records later)
        #   • Count all string occurrences across all classes
        # After the loop:
        #   • Strings with count > 1 go into the global SST
        #   • Column names are always added to the SST (appear in every header row)
        #   • 'Info' is always added (appears in row 1 of every data sheet)
        print(f'  [{ts()}] SST pre-scan ({len(class_names)} classes)...', end='', flush=True)
        t0 = datetime.now()

        META        = ('operation', 'id', 'File_Name', 'Dist_Name', 'SW_Version')
        META_SET    = set(META)
        col_orders  = {}       # cls → (hier_cols, meta_cols, param_cols)
        str_counts  = defaultdict(int)   # str → total occurrence count

        for cls in class_names:
            recs = merged[cls]   # peek only — write loop will pop later
            hier_seen  = {}
            param_seen = {}
            has_op     = False
            ...
            for hier, rec in recs:
                ...
                for k, v in rec.items():
                    if k == 'operation':
                        has_op = True
                    if k not in META_SET:
                        param_seen[k] = True
                    # Count string values for SST decision
            ...
            meta_cols = list(META) if has_op else [m for m in META if m != 'operation']
            col_orders[cls] = (list(hier_seen.keys()), meta_cols, list(param_seen.keys()))

        # Build global SST: column names always in; data strings only if count > 1
        sst_dict = {}   # str → global index
        sst_list = []   # global index → str

        def _sst_add(s):
            if s not in sst_dict:
                sst_dict[s] = len(sst_list)
                sst_list.append(s)

        _sst_add('Info')   # row-1 hyperlink text in every data sheet

        for cls in class_names:
            hier_cols, meta_cols, param_cols = col_orders[cls]
            for col in hier_cols + meta_cols + param_cols:
                _sst_add(col)
        ...
        t_sst = (datetime.now() - t0).total_seconds()
        print(f' {len(sst_list):,} strings ({fmt_elapsed(t_sst)})')
        ...
        with ProcessPoolExecutor(max_workers=max(n_workers, 1)) as ex:
            ...
            for sheet_name, cls, start, end in sheet_plan:
                if cls != prev_cls:
                    if flat_cur is not None:
                        ...
                        flat_cur = None
                    recs = merged.pop(cls)
                    # Use cached column order — no re-scan of records needed
                    hier_cols, meta_cols, param_cols = col_orders[cls]
                    cols_cur  = hier_cols + meta_cols + param_cols
                    nhier_cur = len(hier_cols)
                    flat_cur  = flatten_records_sparse(recs, cols_cur, set(hier_cols))
                    del recs
                    prev_cls = cls

                chunk = flat_cur[start:end] if (end - start) < len(flat_cur) else flat_cur
                # sst_dict is typically small (< 10 000 entries) → cheap to pickle
                args  = (sheet_name, chunk, cols_cur, nhier_cur, temp_dir, sst_dict)
            ...
            while pending:
                ...
                for fut in done_set:
                    sheet_name = futs[fut]
                    try:
                        name_result, sheet_xml_path, rels_path, elapsed = fut.result()
                        sheet_paths[name_result] = (sheet_xml_path, rels_path)
        ...
        assemble_xlsx(sheet_order, sheet_paths, xlsx_path, sst_list)
...
def _ask_summary_dialog(root, has_4g, has_2g, has_hw=False, has_3g=False):
    """
    Show a small Toplevel asking whether to generate summary reports.
    Returns (want_4g: bool, want_2g: bool, want_hw: bool, want_3g: bool).
    All False if user clicks Skip.
    """
    import tkinter as tk

    result = [False, False, False, False]   # [want_4g, want_2g, want_hw, want_3g]
    ...
    var_3g = tk.BooleanVar(value=has_3g)
    ...
    if has_3g:
        tk.Checkbutton(dlg,
                       text='  3G Summary   (RNC/WBTS/WCEL data found)',
                       variable=var_3g,
                       font=('Calibri', 10)).pack(padx=28, anchor='w')
    if has_hw:
        tk.Checkbutton(dlg,
    ...
    def _on_generate():
        ...
        result[3] = bool(var_3g.get()) if has_3g else False
        dlg.destroy()
...
def _run_3g_summary(input_file, output_path, pre_read=None):
    ...
    try:
        from network import Network                  # 3g_tool/network.py
        from reports.cell_summary import build       # 3g_tool/reports/cell_summary.py
        ...
        net   = Network(sheets)
        print(f'[{ts()}] 3G: Building summary -> {os.path.basename(output_path)} ...')
        count = build(net, output_path,
                      progress_fn=lambda m: tprint(f'  [3G] {m}'))
        print(f'[{ts()}] 3G: Done — {count} cells written to '
              f'{os.path.basename(output_path)}')
        return True

    except Exception as exc:
        print(f'[{ts()}] 3G: ERROR — {exc}')
        import traceback; traceback.print_exc()
        return False
...
def _post_process_summaries(output_file, class_names, root):
    ...
    has_3g = {'RNC', 'WBTS', 'WCEL'}.issubset(class_set)
    has_hw = ('INVUNIT' in class_set and
              bool(class_set & {'MRBTS', 'LNBTS', 'LNBTS_FDD', 'LNBTS_TDD'}))

    if not has_4g and not has_2g and not has_3g and not has_hw:
    ...
    want_4g, want_2g, want_hw, want_3g = _ask_summary_dialog(
        root, has_4g, has_2g, has_hw, has_3g)

    if not want_4g and not want_2g and not want_hw and not want_3g:
    ...
    if want_3g:
        out_3g = _unique_path(f'{base}_3G_Summary.xlsx')
        t0 = datetime.now()
        _run_3g_summary(output_file, out_3g, pre_read=pre_read)
        t_3g = (datetime.now() - t0).total_seconds()
...
class ProgressWindow:
    ...
    def __init__(self, parent):
        ...
        self.win.title("OSS XML Parser  V6.5")
...
def run_interactive():
    ...
    _has_3g = {'RNC', 'WBTS', 'WCEL'}.issubset(filter_classes)
    _has_hw = ('INVUNIT' in filter_classes and
               bool(filter_classes & {'MRBTS', 'LNBTS', 'LNBTS_FDD', 'LNBTS_TDD'}))
    want_4g = want_2g = want_3g = want_hw = False
    if _has_4g or _has_2g or _has_3g or _has_hw:
        want_4g, want_2g, want_hw, want_3g = _ask_summary_dialog(
            root, _has_4g, _has_2g, _has_hw, _has_3g)
        if not want_4g and not want_2g and not want_3g and not want_hw:
            print(f'[{ts()}] Summary generation skipped.')
    ...
    _SUMMARY_SHEETS = {
        ...
        'RNC', 'WBTS', 'WCEL', 'WNCEL',   # 3G summary
    }
    ...
    t_4g = t_2g = t_3g = t_hw = 0.0
    if want_4g or want_2g or want_3g or want_hw:
        if want_4g:
        ...
        if want_2g:
            ...
            t_2g = (datetime.now() - t0).total_seconds()
        if want_3g:
            out_3g = _unique_path(f'{summary_base}_3G_Summary.xlsx')
            t0 = datetime.now()
            _run_3g_summary(final_path, out_3g, pre_read=pre_read)
            t_3g = (datetime.now() - t0).total_seconds()
        if want_hw:
    ...
    if t_3g:
        grand_line += f'  |  3G Summary: {fmt_elapsed(t_3g)}'
    if t_hw:
...
def main():
    os.system("title Ankit's XML Parser  [V6.5 - Streaming Parse]")
    ...
    parser = argparse.ArgumentParser(description='OSS XML -> Excel Converter V6.5 (Streaming Parse)')
